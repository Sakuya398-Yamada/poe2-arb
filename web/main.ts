...
/** Same identity as the server's arb.ts loopKey — /api/reference is addressed with these. */
function loopKey(l: Loop) { return `${l.itemId}|${l.from}|${l.to}`; }
...
async function loadRef() {
	...
	try {
		...
		refByKey.clear();
		refCtx = ctx();
		for (const r of ref.loops) refByKey.set(`${r.itemId}|${r.from}|${r.to}`, r);
