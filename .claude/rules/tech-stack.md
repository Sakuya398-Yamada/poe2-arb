...
| 外部データ | GGG Currency Exchange API / RePoE (PoE2) / 公式トレード静的データ(EN/JP) / poe2db（ゴールド手数料） | 認証不要。詳細と URL は `README.md` の「データ源」 |
...
- 環境変数: `PORT`（既定 8765）、`POE2ARB_LEAGUE`（既定 `Forbidden Rites`）、`POE2ARB_UA`（User-Agent）、`POE2ARB_GOLD_PER_EX`（1 高貴あたりのゴールド。設定時のみ手数料込み利益を出す）
